...
export default function CommandDashboard() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr 400px', height: '100vh', padding: '15px', gap: '15px', background: 'var(--bg-color)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
             {vessels.map((v: any) => (
               <div key={v.id} style={{ display: 'flex', flexDirection: 'column', gap: '5px', padding: '15px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', borderLeft: `4px solid ${v.status === 'SOS' ? 'var(--accent-orange)' : 'var(--accent-green)'}`, transition: '0.3s' }}>
                 ...
                 <div style={{ display: 'flex', gap: '15px', fontSize: '0.7rem', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>
                   <span>⚡ {Math.floor(v.battery)}%</span>
                   <span>🌊 {v.speed} kn</span>
                   <span>🧭 {v.heading}°</span>
                 </div>
                 {v.status === 'SOS' && (
                   <div style={{ fontSize: '0.7rem', color: 'var(--accent-orange)', fontWeight: 800, marginTop: '5px', background: 'rgba(255,77,77,0.1)', padding: '5px 8px', borderRadius: '4px' }}>📡 ALERT: {v.lat.toFixed(4)}, {v.lng.toFixed(4)}</div>
                 )}
               </div>
             ))}
          </div>
      ...
      <main style={{ position: 'relative' }}>
        <div className="glass-card" style={{ height: '100%', padding: 0, overflow: 'hidden', border: '1px solid var(--accent-blue-glow)' }}>
          {L && (
          ...
          <div style={{ position: 'absolute', bottom: '25px', left: '25px', zIndex: 1000, display: 'flex', gap: '15px' }}>
             <div className="glass-card" style={{ padding: '12px 25px', display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(5,11,24,0.95)', border: '1px solid var(--accent-blue-glow)' }}>
                <span style={{ fontSize: '1.8rem' }}>🌫️</span>
                <div><div style={{ fontSize: '0.6rem', color: 'var(--text-secondary)', letterSpacing: '0.1em' }}>WAVE HEIGHT</div><div style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--accent-blue)' }}>{incoisData.waveHeight}m</div></div>
             </div>
             <div className="glass-card" style={{ padding: '12px 25px', display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(5,11,24,0.95)', border: '1px solid var(--accent-blue-glow)' }}>
                <span style={{ fontSize: '1.8rem' }}>🌪️</span>
                <div><div style={{ fontSize: '0.6rem', color: 'var(--text-secondary)', letterSpacing: '0.1em' }}>WIND SPEED</div><div style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--accent-blue)' }}>{incoisData.windSpeed}km/h</div></div>
             </div>
          </div>
        </div>
      </main>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                 {sosVessels.map((v: any) => (
                   <div key={`alert-${v.id}`} style={{ padding: '15px', background: 'rgba(255,77,77,0.1)', borderRadius: '16px', border: '1px solid var(--accent-orange)' }}>
                      ...
                      <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.6)', fontFamily: 'var(--font-mono)' }}>TELEMETRY: {v.lat.toFixed(4)}, {v.lng.toFixed(4)}</div>
        ...
        <div className="glass-card" style={{ background: 'rgba(0,255,136,0.03)', borderColor: 'rgba(0,255,136,0.3)' }}>
           ...
           <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <input value={newPFZ.lat} onChange={e => setNewPFZ({...newPFZ, lat: e.target.value})} type="number" placeholder="LAT" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', padding: '12px', borderRadius: '10px', color: 'white', fontSize: '0.8rem', fontFamily: 'var(--font-mono)' }} />
                <input value={newPFZ.lng} onChange={e => setNewPFZ({...newPFZ, lng: e.target.value})} type="number" placeholder="LNG" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', padding: '12px', borderRadius: '10px', color: 'white', fontSize: '0.8rem', fontFamily: 'var(--font-mono)' }} />
              </div>
              <input value={newPFZ.name} onChange={e => setNewPFZ({...newPFZ, name: e.target.value})} placeholder="ZONE NAME (E.G. TUNA HUB)" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', padding: '12px', borderRadius: '10px', color: 'white', fontSize: '0.8rem', fontWeight: 600 }} />
        ...
        <div className="glass-card" style={{ flex: 1, overflowY: 'auto' }}>
           ...
           <div style={{ marginBottom: '20px', padding: '15px', background: 'rgba(0,210,255,0.03)', borderRadius: '16px', border: '1px solid var(--accent-blue-glow)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                <input value={newFish.species} onChange={e => setNewFish({...newFish, species: e.target.value})} placeholder="SPECIES" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', padding: '8px', borderRadius: '8px', fontSize: '0.75rem', color: 'white' }} />
                <input value={newFish.malayalam} onChange={e => setNewFish({...newFish, malayalam: e.target.value})} placeholder="മലയാളം" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', padding: '8px', borderRadius: '8px', fontSize: '0.75rem', color: 'white' }} />
                <input value={newFish.port} onChange={e => setNewFish({...newFish, port: e.target.value})} placeholder="PORT" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', padding: '8px', borderRadius: '8px', fontSize: '0.75rem', color: 'white' }} />
                <input value={newFish.price} onChange={e => setNewFish({...newFish, price: e.target.value})} type="number" placeholder="₹ PRICE" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', padding: '8px', borderRadius: '8px', fontSize: '0.75rem', color: 'white' }} />
              </div>
